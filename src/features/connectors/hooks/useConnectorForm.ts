    ...
    try {
      ...
      if (details) {
        setFormData({
          ...
          power: details.power ?? "",
          power_unit: details.power_unit ?? "KWH",
          time_limit: details.time_limit,
          pin: details.pin ?? "",
          subchargerID: details.subchargerID,
          ...
          stop_on80: !!(details.stop_on80 ?? details.stop_onSoC),
          stop_onSoC: !!(details.stop_onSoC ?? details.stop_on80),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await saveConnector({
        ...
        powerUnit: formData.power_unit || "KWH",
        timeLimit: formData.time_limit,
        pin: formData.pin,
        subchargerID: formData.subchargerID,
        ...
        stopOn80: !!(formData.stop_onSoC ?? formData.stop_on80),
